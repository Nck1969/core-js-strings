...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, firstIndex).concat(str.slice(firstIndex + value.length));
}
...
function countVowels(str) {
  const matchedChars = str.match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  const formattedStr = str.replaceAll(/[, ?!@#$%^&*)(+=._-]/gi, '');
  const isStrLengthEven = formattedStr.length % 2 === 1;

  for (
    let i = 0;
    i <
    (isStrLengthEven ? (formattedStr.length - 1) / 2 : formattedStr.length / 2);
    i += 1
  ) {
    if (
      formattedStr[i].toLowerCase() !==
      formattedStr[formattedStr.length - 1 - i].toLowerCase()
    ) {
      return false;
    }
  }
...
function findLongestWord(sentence) {
  ...
  const wordslengths = splittedSentence.map((word) => word.length);

  return splittedSentence[wordslengths.indexOf(Math.max(...wordslengths))];
}
...
function encodeToRot13(str) {
  ...
  function encodeChar(char) {
    ...
    if (charCode >= aCharCode && charCode <= aCharCode + totalChar) {
      const isA = charCode + 13 > 90;
      if (isA) {
        return String.fromCharCode(aCharCode + (13 - 90 - 1 + charCode));
      }
      return String.fromCharCode(charCode + 13);
    }
    const isB = charCode + 13 > 122;
    if (isB) {
      return String.fromCharCode(ACharCode + (13 - 122 - 1 + charCode));
    }
    return String.fromCharCode(charCode + 13);
  }
  ...
  splittedStr.map((word) => {
    return answer.push(
      word.split('').reduce((newWord, char) => {
        if (char.match(/[.!?@#$%^&*()_+-=]/gi) === null) {
          return newWord.concat(encodeChar(char));
        }
        return newWord.concat(char);
      }, '')
    );
  });
...
function getCardId(value) {
  ...
  switch (cardValue) {
    ...
    case 'K':
      ...
      break;
    default:
      cardIndex = +cardValue - 1;
      break;
  }

  return playingCardSuits.indexOf(cardSuit) * 13 + cardIndex;
}
